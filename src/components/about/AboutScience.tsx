...
const DEFAULT_TOPICS = [
  { step: "01", title: "Pure Mycelium Culture", description: "Isolated on sterile agar media slants and verified through phase contrast microscopic analysis for complete genetic authenticity.", icon: FlaskConical },
  { step: "02", title: "Liquid Spawn Technology", description: "Active vegetative mycelium suspended in sterile liquid broth, engineered for 4x faster substrate colonization.", icon: Droplets },
  { step: "03", title: "Genetic Stability", description: "Multi-generation testing and strain preservation prevent genetic degeneration across commercial multiplication cycles.", icon: Dna },
  { step: "04", title: "Laboratory Verification", description: "Rigorous quality inspection including microscopic sectoring checks and bio-efficiency purity validation.", icon: Microscope },
  { step: "05", title: "Contamination Control", description: "Class 100 HEPA-filtered cleanrooms equipped with positive atmospheric pressure and continuous UV sterilization.", icon: ShieldCheck },
  { step: "06", title: "Cold Chain Storage", description: "Strict temperature-regulated cold chain distribution maintains mycelial dormancy until direct farm inoculation.", icon: Thermometer }
];

export default function AboutScience() {
  const { data } = useCMS();
  const rawCards = data?.about?.scienceCards || [];
  const iconsList = [FlaskConical, Droplets, Dna, Microscope, ShieldCheck, Thermometer];
  
  const activeTopics = rawCards.length > 0
    ? rawCards.map((c, idx) => ({
