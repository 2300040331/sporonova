...
const DEFAULT_QUALITY = [
  { id: 1, title: "GMP Laboratory", desc: "Good Manufacturing Practice compliant facility ensuring contamination-free production", Icon: Building2 },
  { id: 2, title: "ISO Standards", desc: "International quality management standards across all processes", Icon: Award },
  { id: 3, title: "DMR Certification", desc: "Directorate of Mushroom Research validated protocols", Icon: FileCheck },
  { id: 4, title: "NHB Certification", desc: "National Horticulture Board quality standards compliance", Icon: BadgeCheck },
  { id: 5, title: "Scientific Validation", desc: "Rigorous strain verification and biological efficiency testing", Icon: FlaskConical },
];
...
export default function AboutQuality() {
  const { data } = useCMS();
  const rawQuality = data?.about?.qualityCards || [];
  const iconSet = [Building2, Award, FileCheck, BadgeCheck, FlaskConical];

  const activeQuality = rawQuality.length > 0
    ? rawQuality.map((q, idx) => ({
        id: idx + 1,
        title: q.title,
        desc: q.description,
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            {activeQuality.slice(0, 3).map((item) => (
              <motion.div
                key={item.id}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {activeQuality.slice(3).map((item) => (
              <motion.div
                key={item.id}
                variants={itemVariants}
                className="group relative about-glow-border rounded-3xl"
