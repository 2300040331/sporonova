...
const DEFAULT_PARTNERSHIPS = [
  { icon: Landmark, title: "Government Institutions", description: "Working with state horticulture departments and JICA for farmer empowerment programs." },
  { icon: Microscope, title: "Research Organizations", description: "Collaborating with ICAR-DMR and biotechnology research institutes." },
  { icon: GraduationCap, title: "Agricultural Universities", description: "Supplying research-grade cultures and training materials to academic institutions." },
  { icon: HandshakeIcon, title: "Farmer Cooperatives", description: "Supporting FPO formation and providing technical assistance to farming communities." },
  { icon: Globe, title: "International Development Partners", description: "Partnering with global organizations for sustainable agricultural development." }
];
...
export default function AboutPartnerships() {
  const { data } = useCMS();
  const rawPartners = data?.about?.partnershipCards || [];
  const iconList = [Landmark, Microscope, GraduationCap, HandshakeIcon, Globe];

  const activePartnerships = rawPartners.length > 0
    ? rawPartners.map((p, idx) => ({
