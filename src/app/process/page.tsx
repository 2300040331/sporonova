...
export default function ProcessPage() {
  ...
  const stepsList: ProcessStage[] = (stepsFromCMS && stepsFromCMS.length > 0)
    ? stepsFromCMS.map((st: any, i: number) => ({
        step: st.step || (i + 1 < 10 ? `0${i + 1}` : `${i + 1}`),
        name: st.title || st.name || "",
        purpose: st.purpose || st.description || "",
        equipment: Array.isArray(st.equipment) ? st.equipment : (st.details || []),
        temp: st.temp,
        time: st.time || "Daily",
        pressure: st.pressure,
        precautions: st.precautions || "Maintain strict aseptic laboratory protocols.",
        qualityCheck: st.qualityCheck || "Verified 100% pure.",
      }))
    : LIQUID_SPAWN_PROCESS;
  return (
    <>
      <Navbar />
      <main 
